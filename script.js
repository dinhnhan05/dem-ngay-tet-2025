...
function seqSmallBarrage() {
  ...
  function launchShell(x, useSpecial) {
    const isRandom = shellNameSelector() === 'Ngẫu';
    let shellType = isRandom
      ? useSpecial ? randomSpecialShell : randomMainShell
      : shellTypes[shellNameSelector()];
...
// Thiết lập ngày đích (29 tháng 1, 2025)
const countDownDate = new Date("Jan 29, 2025 00:00:00").getTime();
...
const x = setInterval(function() {
  ...
  document.getElementById("count-time").innerHTML = days + " ngày " + hours + " giờ " + minutes + " phút " + seconds + " giây ";

  // Nếu thời gian đếm ngược kết thúc, hiển thị thông báo
  if (distance < 0) {
    clearInterval(x);
    document.getElementById("count-time").innerHTML = "Chúc mừng năm mới!!!";
  }
}, 1000);